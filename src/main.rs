...
fn run_migrate(args: &[String]) {
    ...
    let env_or = |k: &str, default: &str| std::env::var(k).unwrap_or_else(|_| default.to_string());

    // ── Required ──────────────────────────────────────────────────────────────
    let target_url = match env("TARGET_URL") {
        Some(u) => u,
        None => {
            eprintln!("migrate: TARGET_URL is not set — nothing to migrate.");
            std::process::exit(1);
        }
    };

    // ── Load model ────────────────────────────────────────────────────────────
    let model_type = env_or("LOAD_MODEL_TYPE", "Concurrent");
    let (load_model_str, target_rps_line) = match model_type.to_lowercase().as_str() {
        "rps" => {
            let rps = env_or("TARGET_RPS", "10");
            ("rps".to_string(), format!("  target: {}", rps))
        }
        "ramprps" => {
            let min = env_or("MIN_RPS", "1");
            let max = env_or("MAX_RPS", "100");
            let ramp = env_or("RAMP_DURATION", "");
            let ramp_line = if ramp.is_empty() {
                String::new()
            } else {
                format!("\n  rampDuration: \"{}\"", ramp)
            };
            (
                "rampRps".to_string(),
                format!("  minRps: {}\n  maxRps: {}{}", min, max, ramp_line),
            )
        }
        _ => ("concurrent".to_string(), String::new()),
    };

    // ── Workers / duration / timeout ──────────────────────────────────────────
    let workers = env_or("NUM_CONCURRENT_TASKS", "10");
    let duration = env_or("TEST_DURATION", "2h");
    let timeout = env_or("REQUEST_TIMEOUT_SECS", "30");
    let timeout_str = format!("{}s", timeout);
    let skip_tls = env_or("SKIP_TLS_VERIFY", "false");

    // ── Request ───────────────────────────────────────────────────────────────
    let method = env_or("REQUEST_TYPE", "GET");
    let send_json = env_or("SEND_JSON", "false");
    let json_payload = env("JSON_PAYLOAD");

    // ── Body section for single-URL config ────────────────────────────────────
    let body_section = if send_json == "true" {
        match &json_payload {
            Some(payload) => format!(
    ...
    // ── Optional metadata ─────────────────────────────────────────────────────
    let node_id = env("CLUSTER_NODE_ID").unwrap_or_default();
    let region = env_or("CLUSTER_REGION", "local");
    let tenant = env("TENANT");

    let mut metadata_extras = String::new();
    if !node_id.is_empty() {
        metadata_extras.push_str(&format!("\n  author: \"{}\"", node_id));
    }
...
metadata:
  ...
  description: "Generated by rust-loadtest migrate from environment variables"
  region: "{region}"{metadata_extras}
...
"#,
    ...
    match std::fs::write(output_path, &yaml) {
        Ok(()) => {
            eprintln!("migrate: wrote YAML config to '{}'", output_path);
        ...
        Err(e) => {
            eprintln!("migrate: failed to write '{}': {}", output_path, e);
            std::process::exit(1);
        }
    }
    std::process::exit(0);
